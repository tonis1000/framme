...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Laden der Playlist-Datei
    function loadPlaylist(url) {
        fetch(url)
            .then(response => response.text())
            .then(data => {
                const lines = data.split('\n');
                sidebarList.innerHTML = ''; // Liste zurücksetzen
    ...
    function addChannelToSidebar(channel) {
        ...
        li.addEventListener('click', () => {
            playStream(channel.url, channel.name, channel.logo);
        });
    ...
    playButton.addEventListener('click', () => {
        ...
        if (url) {
            playStream(url, 'Benutzerdefiniert', '');
        }
    });
    ...
    subtitleFileInput.addEventListener('change', () => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                const subtitleTrack = document.getElementById('subtitle-track');
                subtitleTrack.src = URL.createObjectURL(new Blob([reader.result], { type: 'text/vtt' }));
    ...
    document.getElementById('myPlaylist').addEventListener('click', () => {
        loadPlaylist('playlist.m3u'); // Lokale Playlist
    });
    document.getElementById('externalPlaylist').addEventListener('click', () => {
        loadPlaylist('https://example.com/external-playlist.m3u'); // Externe Playlist
    });
